...
const ChatUtils = {
  ...
  // 处理消息发送逻辑
  handleMessageSend: function(options) {
    ...
    // 创建用户消息
    const userMessage = this.createUserMessage(validation.text)
    ...
    return ApiUtils.dialogue.sendMessage(validation.text, character.id, conversationId).then(function(result) {
      // 更新用户消息状态
      const messagesWithStatus = ChatUtils.updateMessageStatus(updatedMessages, userMessage.id, 'sent')
      
      if (result.success) {
        // 创建AI回复
        const aiMessage = ChatUtils.createAIMessage(result.data.response)
        const finalMessages = messagesWithStatus.concat([aiMessage])
      ...
      // 添加错误消息
      const errorMessage = ChatUtils.createErrorMessage()
