...
export default function LearningCenter() {
  ...
  const fetchApiStatus = async () => {
    try {
      const response = await axios.get('/api/v1/api-settings/current');
      setApiStatus(response.data);
    } catch (err) {
      console.error('Error fetching API status:', err);
      notification.error({
        message: 'API状态获取失败',
        description: '无法获取当前AI服务状态，可能需要配置API密钥。',
      });
    }
  };
  ...
  const createNewConversation = async (scholarId: string, firstMessage: string) => {
    setLoading(true);
    try {
      const persona = scholarMap[scholarId as keyof typeof scholarMap] || '现代导师';
  ...
  const sendMessageToConversation = async (conversationId: number, message: string) => {
    setLoading(true);
    try {
      const response = await axios.post(`/api/v1/conversations/${conversationId}/messages`, {
        message: message
      });
      
      const updatedConversation = response.data;
      setCurrentConversation(updatedConversation);
      setDialogues(updatedConversation.messages);
